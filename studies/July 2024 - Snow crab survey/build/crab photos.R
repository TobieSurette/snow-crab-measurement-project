...
file <- data.frame(name = dir(path = "W:/Crab/Offshore Crab Common/Fishing Year 2024/Trawl Survey/photos/crab photos",
                   full.names = TRUE, include.dirs = FALSE))

file$name <- gsub(" +[Cc]rab +#", " crab #", file$name)
file$name <- gsub(" +", " ", file$name)
file$name <- gsub("[#] +", "# ", file$name)
file <- data.frame(name = file$name[grep(" crab ", file$name)])
